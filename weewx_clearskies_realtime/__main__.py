...
Startup order (per spec):
  ...
  4. Validate mode == "mqtt"
  5. Lazy-import paho-mqtt; fail clearly if absent
...
def _resolve_bind_addresses(host: str, port: int) -> list[tuple[str, int]]:
    ...
    hosts_to_resolve: list[str] = (
        ["127.0.0.1", "::1"] if host in loopback_aliases else [host]
    )
...
async def serve_all(
    ...
    mqtt_adapter: Any,  # MQTTAdapter
    sse_emitter: Any,  # SSEEmitter
) -> None:
    """Start MQTT, fan-out, and both uvicorn servers; run until SIGTERM/SIGINT."""
    import uvicorn  # type: ignore[import-untyped]

    loop = asyncio.get_running_loop()

    # --- Start MQTT adapter (runs paho thread internally) ---
    mqtt_adapter.start(loop)
...
def main() -> None:
    ...
    if settings.input.mode != "mqtt":
        logger.error(
            "Only 'mqtt' mode is supported in this release. "
            "Direct mode is not yet implemented. Set [input] mode = mqtt."
        )
        sys.exit(1)

    # Step 5: lazy-import paho-mqtt; fail clearly.
    try:
    ...
    # Step 6: build queue, adapter, emitter, apps.
    ...
    mqtt_adapter = MQTTAdapter(settings.mqtt, packet_queue)
    sse_emitter = SSEEmitter(packet_queue)

    main_app = create_app(settings, sse_emitter)
    health_app = create_health_app()

    # Register MQTT health probe.
    register_readiness_probe(mqtt_adapter.health_probe)

    logger.info(
        "Starting weewx-clearskies-realtime",
        extra={
    ...
    with contextlib.suppress(KeyboardInterrupt):
        asyncio.run(
            serve_all(main_app, health_app, settings, mqtt_adapter, sse_emitter)
        )
